...
def test_user_facing_paths() -> None:
    assert is_user_facing("crates/mnemosyne-store/src/commit.rs")
    assert is_user_facing("python/mnem/_sdk.py")
    assert is_user_facing("docs/format/README.md")
    assert is_user_facing("README.md")
    assert not is_user_facing("crates/mnemosyne-store/tests/round_trip.rs")
...
def test_changelog_check_flags_a_missing_update() -> None:
    assert changelog_check(["crates/mnemosyne-git/src/main.rs"])
    assert changelog_check(["crates/mnemosyne-git/src/main.rs", "CHANGELOG.md"]) == []
