...
<p align="center">
  <img src="assets/demo.gif" alt="An agent records a belief with provenance, forks its memory to test a hunch, then uses bisect and blame to find the commit where a misread observation entered." width="900" />
</p>

An AI agent builds up memory as it works — facts it learns, decisions it makes — and frameworks store that as state it overwrites as it goes. Mnemosyne gives agent memory what Git gives code: **commits, branches, merge, blame and bisect.** A Rust core, a `mnem` CLI, and a Python SDK. Local, deterministic, no network, no model calls.
