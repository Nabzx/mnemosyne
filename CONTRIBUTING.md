...
**One real exception to "re-land the bump": `.github/workflows/release.yml`.**
That file is autogenerated by `cargo-dist` (its own header comment says so) -
hand-editing it, including re-landing a Dependabot bump to an action version
inside it, desyncs it from what `dist generate --mode ci` would produce for
the pinned `cargo-dist-version` in `dist-workspace.toml`, and `dist plan`'s
own self-consistency check then fails on every PR (found and fixed the hard
way, #325). A Dependabot PR touching this file specifically should be closed
without re-landing; if the action versions inside it genuinely need bumping,
bump `cargo-dist-version` instead and run `dist generate --mode ci` to
regenerate the whole file properly.
