...
//!
//! This is a thin binding over `mnem-store` (ADR-0004): it exposes the store and
//! the commit graph to Python and maps [`mnem_store::MnemError`] onto a Python
...
/// A Mnemosyne store. Wraps [`mnem_store::Store`].
...
impl PyStore {
    ...
    /// `mnem_store::Provenance` (ADR-0003). Returns the node object's id as hex.
    #[pyo3(signature = (
        id,
        content,
        *,
...
fn _mnem(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add("__version__", mnem_store::version())?;
