...
use mnem_store::bisect::{node_content_is, node_present};
use mnem_store::{ContentKind, MemoryNode, Provenance, Store};
...
fn buggy_run() -> (Store, std::path::PathBuf, mnem_store::ObjectId) {
    let root = scratch();
...
fn bisect_lands_on_the_bad_commit_and_blame_explains_it() {
    ...
    assert_eq!(
        ...
        Some(&mnem_store::ChangeKind::Modified)
    );
