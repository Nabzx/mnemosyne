...
//!
//! This is a thin binding over `mnemosyne-store` (ADR-0004): it exposes the store and
//! the commit graph to Python and maps [`mnemosyne_store::MnemError`] onto a Python
...
/// A Mnemosyne store. Wraps [`mnemosyne_store::Store`].
...
impl PyStore {
    ...
    /// `mnemosyne_store::Provenance` (ADR-0003). Returns the node object's id as hex.
    #[pyo3(signature = (
        id,
        content,
        *,
...
fn version() -> &'static str {
    mnemosyne_store::version()
}

#[pymodule]
fn _mnem(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add("__version__", mnemosyne_store::version())?;
