...
```bash
cargo install mnemosyne-git   # installs the `mnem` binary
...
The crate is named `mnemosyne-git`; the installed binary is `mnem`. The library
core is [`mnemosyne-store`](https://crates.io/crates/mnemosyne-store); the Python
SDK is `mnemosyne-agents` on PyPI.
